...
1. **A placeholder MUST NOT contain a line terminator.** A `{{ … }}` construct
   ...
   read as message text at all (section 5).
...
4. **An option's colon is the first unescaped colon in the segment.**
   ...
   therefore contain unescaped colons.
...
`false` or the host's null MUST be treated as present. A value that is present
but that no conversion can describe (section 4) is treated as absent.
...
absent value takes the fallback chain.
...
to the fallback chain and SHOULD report the failure.
...
names a modifier under it, that modifier answers as it did.
...
to the fallback chain and the failure SHOULD be reported.
